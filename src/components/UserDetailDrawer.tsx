    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-4xl overflow-y-auto flex flex-col h-full">
        ...
        <div className="mt-6 space-y-4 flex-1">
          ...
          {/* Invited Users - users who have been invited but haven't logged in yet */}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className="text-xs">Work Email</TableHead>
                        <TableHead className="text-xs">Role</TableHead>
                        <TableHead className="text-xs">Invited On</TableHead>
                    ...
                    <TableBody>
                      {invitedUsers.map((invite) => {
                        const fullName = invite.first_name && invite.last_name
                          ...
                          : invite.first_name || invite.last_name || "—";
                        return (
                          <TableRow key={invite.invite_id || invite.email} className="text-sm">
                            <TableCell className="font-medium truncate max-w-[150px]">{fullName}</TableCell>
                            <TableCell className="truncate max-w-[200px] text-muted-foreground">{invite.email}</TableCell>
                            <TableCell>{getRoleBadge(invite.role)}</TableCell>
                            <TableCell className="text-muted-foreground">{formatDateOnly(invite.created_at)}</TableCell>
                            <TableCell>
                              <Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200">
                                Pending
                              </Badge>
