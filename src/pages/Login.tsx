...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await loginSuperAdmin({ email: email.toLowerCase().trim(), password }, dispatch);
      if (response.challenge_name === "NEW_PASSWORD_REQUIRED") {
        navigate("/force-change-password", { state: { session: response.session, email: response.email } });
        return;
      }
      const user = {
        id: response.user_id,
        email: response.email,
        name: response.display_name || `${response.firstName || ""} ${response.lastName || ""}`.trim(),
      ...
      const errorMessage =
        backendMessage || err.message || "Login failed. Please check your credentials and try again.";
      ...
      <div className="w-full max-w-md animate-scale-in">
        ...
        <Card className="card-shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Sign in with your Super Admin account (email and password only)</CardDescription>
          </CardHeader>
          <CardContent>
            {error && (
