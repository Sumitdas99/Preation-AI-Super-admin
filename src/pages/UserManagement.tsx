...
export default function UserManagement() {
  ...
  const [invitedUsersForDrawer, setInvitedUsersForDrawer] = useState([]);
  ...
  useEffect(() => {
    ...
    // Fetch invited users (pending invitations) if we have brand_id
    let brandId = selectedUser.brand_id;
    
    // If brand_id is not available, try to look it up from brandsList by brand_name
    if (!brandId && (selectedUser.brand_name || selectedUser.workspace) && brandsList.length > 0) {
      const brandName = selectedUser.brand_name || selectedUser.workspace;
      const brand = brandsList.find(
        (b) => (b.brand_name || "").trim().toLowerCase() === brandName.trim().toLowerCase()
      );
      if (brand) {
        brandId = typeof brand.brand_id === "string" ? brand.brand_id : String(brand.brand_id);
      }
    }
    
    if (brandId) {      
      const brandIdStr = typeof brandId === 'string' 
        ? brandId 
        : String(brandId);
      adminApi.getInvitationsByBrand(brandIdStr)
        .then((data) => {
          // Filter to only show invitations with accepted_at = null (pending)
          const pendingInvites = (data.invitations || []).filter(inv => inv.accepted_at === null);
          setInvitedUsersForDrawer(pendingInvites);
        })
        .catch(() => setInvitedUsersForDrawer([]));
    } else {
      setInvitedUsersForDrawer([]);
    }
  }, [isDrawerOpen, selectedUser?.brand_name, selectedUser?.workspace, selectedUser?.brand_id, brandsList]);

  const handleViewUser = (user) => {
    // Try to get brand_id from user object, or look it up from brandsList by brand_name
    let brandId = user.brand_id;
    if (!brandId && (user.brand_name || user.workspace)) {
      const brandName = user.brand_name || user.workspace;
      const brand = brandsList.find(
        (b) => (b.brand_name || "").trim().toLowerCase() === brandName.trim().toLowerCase()
      );
      if (brand) {
  ...
  // State for invited Brand Admin users
  const [invitedBrandAdmins, setInvitedBrandAdmins] = useState([]);
  const [loadingInvitedBrandAdmins, setLoadingInvitedBrandAdmins] = useState(false);
  const [invitedBrandAdminsPage, setInvitedBrandAdminsPage] = useState(1);
  const [invitedBrandAdminsPageSize, setInvitedBrandAdminsPageSize] = useState(10);
  const [invitedBrandAdminsTotal, setInvitedBrandAdminsTotal] = useState(0);
  ...
  // Fetch invited Brand Admin users
  useEffect(() => {
    fetchInvitedBrandAdmins();
  }, [invitedBrandAdminsPage, invitedBrandAdminsPageSize]);

  const fetchInvitedBrandAdmins = async () => {
    setLoadingInvitedBrandAdmins(true);
    try {
      const data = await adminApi.getAllBrandAdminInvitations({
        page: invitedBrandAdminsPage,
        limit: invitedBrandAdminsPageSize,
      });
      const invitations = data.invitations || [];
      setInvitedBrandAdmins(invitations);
      setInvitedBrandAdminsTotal(data.total || 0);
    } catch (err) {
      console.error("Failed to fetch invited brand admins:", err);
      setInvitedBrandAdmins([]);
      setInvitedBrandAdminsTotal(0);
    } finally {
      setLoadingInvitedBrandAdmins(false);
    }
  };
    ...
    <div className="space-y-4 p-6 animate-fade-in relative">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <CreateBrandAdminDialog
            ...
            onSuccess={fetchInvitedBrandAdmins}
          />
      ...
      {selectedUser && (
        <UserDetailDrawer
          ...
          invitedUsers={invitedUsersForDrawer}
        />
      )}
      ...
      <Card className="card-shadow">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {invitedBrandAdminsTotal} pending Brand Admin invitation{invitedBrandAdminsTotal !== 1 ? "s" : ""} found
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-muted-foreground">Page size:</span>
              <Select value={invitedBrandAdminsPageSize.toString()} onValueChange={(v) => {
                setInvitedBrandAdminsPageSize(Number(v));
                setInvitedBrandAdminsPage(1);
              }}>
                <SelectTrigger className="w-20">
                  <SelectValue />
                </SelectTrigger>
        ...
        <CardContent>
          {loadingInvitedBrandAdmins ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              <Loader2 className="h-8 w-8 animate-spin text-primary" />
              <p className="text-sm text-muted-foreground">Loading invited users...</p>
            </div>
          ) : invitedBrandAdmins.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <AlertCircle className="h-12 w-12 text-muted-foreground mb-4" />
              <h3 className="text-lg font-semibold mb-2">No invited users found</h3>
              <p className="text-sm text-muted-foreground max-w-sm">
                No pending Brand Admin invitations found.
              </p>
            </div>
          ) : (
            <>
              ...
              {Math.ceil(invitedBrandAdminsTotal / invitedBrandAdminsPageSize) > 1 && (
                <div className="mt-4 flex items-center justify-between">
                  <p className="text-sm text-muted-foreground">
                    Showing {(invitedBrandAdminsPage - 1) * invitedBrandAdminsPageSize + 1}–
                    {Math.min(invitedBrandAdminsPage * invitedBrandAdminsPageSize, invitedBrandAdminsTotal)} of{" "}
                    {invitedBrandAdminsTotal} invitations
                  </p>
                  <div className="flex items-center space-x-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setInvitedBrandAdminsPage(p => Math.max(1, p - 1))}
                      disabled={invitedBrandAdminsPage === 1}
                    >
                      Previous
                    </Button>
                    <span className="text-sm">Page {invitedBrandAdminsPage} of {Math.ceil(invitedBrandAdminsTotal / invitedBrandAdminsPageSize)}</span>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setInvitedBrandAdminsPage(p => Math.min(Math.ceil(invitedBrandAdminsTotal / invitedBrandAdminsPageSize), p + 1))}
                      disabled={invitedBrandAdminsPage >= Math.ceil(invitedBrandAdminsTotal / invitedBrandAdminsPageSize)}
                    >
                      Next
                    </Button>
                  </div>
