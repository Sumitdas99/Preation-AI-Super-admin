...
import { Shield, Loader2, ArrowLeft, Check, Eye, EyeOff } from "lucide-react";
...
function Requirement({ label, met }: { label: string; met: boolean }) {
  return (
    <div className={`flex items-center text-xs ${met ? "text-green-600" : "text-muted-foreground"}`}>
      {met ? <Check className="mr-1 h-3 w-3" /> : <div className="mr-1 h-1 w-1 rounded-full bg-muted-foreground/50" />}
      {label}
    </div>
  );
}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    className="pr-10"
                  />
                  <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-2.5 text-muted-foreground hover:text-foreground" tabIndex={-1}>
                    {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </button>
                </div>
              </div>
              <Button type="submit" className="w-full bg-gradient-primary" disabled={isLoading || !allChecksPassed}>
