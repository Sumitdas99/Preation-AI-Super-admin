...
export default function ForceChangePassword() {
  ...
  useEffect(() => {
    if (!session || !email) navigate("/login");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await completeNewPasswordChallenge({ session, email, new_password: newPassword }, dispatch);
      const user = {
        id: response.user_id,
      ...
      <div className="w-full max-w-md animate-scale-in">
        ...
        <Card className="card-shadow-lg">
          <CardHeader>
            ...
            <CardDescription>For your security, you must change your temporary password.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" className="w-full bg-gradient-primary" disabled={isLoading}>
                {isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Updating Password...</>) : "Set Password & Login"}
              </Button>
            </form>
          </CardContent>
        </Card>
